...
| 한계 | 실측 | 어떻게 대처하나 |
|---|---|---|
| **실제 사람 손글씨 스타일은 재현도가 낮다** | 학습 writer 가 전부 **폰트 99종**이다. 릴리즈 모델은 `online_split`(폰트 합성)으로만 학습했고 실제 손글씨 데이터(HanDB 등, 코드는 있음)는 **안 썼다** | 폰트는 같은 글자를 항상 같게 쓰지만 사람 손은 매번 다르다 — 글자마다 달라지는 흔들림·연결·필압을 **배운 적이 없다**. 실제 손글씨를 style 로 주면 (bbox 크롭 후) 읽히기는 해도 그 사람 필체의 재현도는 폰트 style 대비 확실히 떨어진다 |
...
| **겹받침(ㄳㄵㄺ…)이 특히 나쁘다** | 통제 조건에서 음절정확도 **0.493** vs 홑받침 0.711 (−22%p) | 원인은 받침 개수가 아니라 **글리프 밀도**. 조사 진행 중 ([JONGSEONG](docs/JONGSEONG_STROKE_LOSS.md)) |
| **스캔 이미지를 그냥 넣으면 생성이 무너진다** (위와 별개 원인 — 입력 조건 쪽) | 여백이 latent std 를 폰트의 **6~29배**로 튀겨 runaway/blank | `--bbox-crop`(잉크 bbox 크롭)으로 해결됨. 적용 시 4모델×8샘플 32/32 legible ([VAE_ROBUSTNESS](docs/VAE_ROBUSTNESS.md)) |
| **흰 배경 + 검은 글씨만 나온다** | 학습 데이터 정책이 gen(타깃)에 **흰 배경 강제**·ColorJitter brightness=0·RandomInvert 비활성이고, 출력은 1채널 그레이스케일이다 | 종이 질감·색 잉크·배경 이미지는 **모델이 못 만든다**. style 에 배경이 있어도 글자 모양만 전달되고 배경은 안 따라온다 — 필요하면 생성 후 합성으로 입힐 것 |
...
| **writer 다양성이 upstream 의 1/1000 수준** | 학습 writer = 한글 폰트 **83종** + 라틴 177종 (별도 held-out 16종은 평가 전용). upstream 은 ~100k writer | 처음 보는 극단 장식/붓 계열 폰트는 품질이 떨어진다. 스타일 일반화 기대치를 낮출 것 |
