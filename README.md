...
experiments/_eval_jong_stroke_loss.py  # 종성(받침) 획 손실 — VAE roundtrip 을 음절 창 단위로 픽셀 정밀 측정
experiments/_eval_jong_gen_loss.py     # 종성 획 손실 — 생성물을 HTR 로 읽어 자모 단위(삭제/오치환) 측정
experiments/_eval_jong_hypotheses.py   # 위 두 덤프 재분석 — 획 수(H1)/학습 빈도(H2) 가설 검증(GPU 불필요)
# 모델 / 원본 코드
...
### ⚠️ 알려진 이슈: 종성(받침) 획 손실 — 특히 **겹받침** (2026-08, 조사 진행 중)

받침 있는 음절에서 획이 뭉개지는 현상을 정량화했다. 상세·중간 실험은
[`docs/JONGSEONG_STROKE_LOSS.md`](docs/JONGSEONG_STROKE_LOSS.md), 측정 스크립트는
`experiments/_eval_jong_stroke_loss.py`(VAE 픽셀 정밀) / `_eval_jong_gen_loss.py`(생성, 자모 단위).
