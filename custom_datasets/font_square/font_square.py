...
def get_fonts(fonts_path):
    ...
    if isinstance(fonts, Path) and fonts.is_dir():
        exts = {'.ttf', '.otf', '.ttc'}
        fonts = sorted([p for p in fonts.rglob('*') if p.is_file() and p.suffix.lower() in exts])
...
def make_renderers(fonts, height=None, width=None, calib_text=None, calib_threshold=0.7, calib_h=128, verbose=False, load_font_into_mem=False, 
    ...
    def load_fonts_parallel(fonts_path, load_font_into_mem, num_threads, verbose=True):
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            ...
            for future in tqdm(as_completed(futures), total=len(futures), desc='Loading font batches', disable=not verbose):
    ...
    if verbose:
        try:
            print(f"Loaded {len(renderers)} fonts")
        except Exception:
            pass
    return renderers
...
class OnlineFontSquare(Dataset):
    def __init__(self, fonts, backgrounds, text_sampler=None, transform=None, length=None, load_font_into_mem=False, renderers=None, max_fonts=None, **kwargs):
        ...
        if renderers is None:
            renderers = make_renderers(self.fonts, calib_threshold=0.8, verbose=True, load_font_into_mem=load_font_into_mem, num_threads=kwargs.get('num_threads', 8))
        try:
            renderers = sorted(renderers, key=lambda r: len(r.charset), reverse=True)
        except:
            pass
        renderers = renderers[:100000]
        # Track the number of available renderers to avoid index errors when some fonts fail to load
        self._num_renderers = len(renderers)
    ...
    def __getitem__(self, font_id):
        font_id = font_id % (self._num_renderers if hasattr(self, '_num_renderers') else len(self.fonts))
...
class HFDataCollector:
    ...
    def __call__(self, batch):
        ...
        res['img'] = pad_images([sample['text_img'].repeat(3, 1, 1) for sample in batch])
        return res
...
class TextSampler:
    ...
    def load_words(self, words_dict_path):
        if Path(words_dict_path).exists():
            ...
            assert len(self.words) == 103452, 'Words count mismatch. Expected 103452 words.'
