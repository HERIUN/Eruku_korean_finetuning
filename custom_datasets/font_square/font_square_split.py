...
from . import font_transforms_split as FT
...
def collate_fn(batch):
    ...
    for item in batch:
        ...
        style_texts.append(item['style_text'])
        gen_texts.append(item['gen_text'])
        style_img_widths.append(item['style _img_width'])
...
class OnlineSplitFontSquare(Dataset):
    def __init__(self, fonts, backgrounds, text_sampler=None, transform=None, length=None, load_font_into_mem=False, renderers=None, max_fonts=None, **kwargs):
        ...
        self.renderers_length = len(renderers)

        self.length = self.renderers_length if length is None else length
    ...
    def __getitem__(self, font_id):
        ...
        style_text, gen_text = self.text_sampler(), self.text_sampler()
        ...
        # style_text_logits_ctc, gen_text_logits_ctc = self.alphabet.encode(style_text), self.alphabet.encode(gen_text)
        # style_text_logits_s2s, gen_text_logits_s2s = torch.cat([sos, style_text_logits_ctc, eos]), torch.cat([sos, gen_text_logits_ctc, eos])
        unpadded_text_len = len(sample['style_text'] + sample['gen_text'])
        style_img_width = sample['style_img_width']*sample['img'].shape[2]//sample['total_img_width']
        gen_img_width = sample['gen_img_width']*sample['img'].shape[2]//sample['total_img_width']
        

        return {
            'style_text_img': sample['text_img'][:, :, :style_img_width],
            'gen_text_img': sample['text_img'][:, :, style_img_width:],
            'style_img': sample['img'][:, :, :style_img_width],
            'gen_img': sample['img'][:, :, style_img_width:],
            'text': sample['style_text'] + sample['gen_text'],
            'style_text': sample['style_text'],
            'gen_text': sample['gen_text'],
            'style_img_width': style_img_width,
            'gen_img_width': gen_img_width,
            'writer': font_id,
...
class HFSplitDataCollector:
    def __init__(self, tokenizer):
        self.tokenizer = tokenizer

    def __call__(self, batch):
        style_txts = [sample['style_text'] for sample in batch]
        gen_txts = [sample['gen_text'] for sample in batch]
        style_img_widths = [sample['style_img_width'] for sample in batch]

        res = {}
        res['style_text'] = style_txts
        res['gen_text'] = gen_txts
        res['style_img_width'] = style_img_widths
        

        res['style_img'] = pad_images([sample['style_text_img'].repeat(3, 1, 1) for sample in batch])
        res['gen_img'] = pad_images([sample['gen_text_img'].repeat(3, 1, 1) for sample in batch])
        res['style_img_width'] = [sample['style_img_width'] for sample in batch]
        res['gen_img_width'] = torch.tensor([sample['gen_img_width'] for sample in batch])
        return res
