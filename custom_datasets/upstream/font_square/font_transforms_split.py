...
class RenderImage(object):
    ...
    def render_raw(self, text, render_class):
        return render_class.render(text, action='top_left', pad=self.pad)

    def render_sample(self, sample, render_class):
        np_style_img, sample['style_text'] = self.render_raw(sample['style_text'], render_class)
        np_gen_img, sample['gen_text'] = self.render_raw(sample['gen_text'], render_class)

        return np_style_img, np_gen_img

    def clean_text(self, text):
        return ''.join([c for c in text if c in set(string.ascii_lowercase + ' ')])

    def convert_whitespace_to_space(self, text):
        return ' '.join(text.split())

    def clean_sample(self, sample):
        sample['style_text'] = self.clean_text(sample['style_text'])
        sample['style_text'] = self.convert_whitespace_to_space(sample['style_text'])
        sample['gen_text'] = self.clean_text(sample['gen_text'])
        sample['gen_text'] = self.convert_whitespace_to_space(sample['gen_text'])
        return sample

    def resize_and_fill_with_white(self, img, height):
        return np.pad(img, ((0, height - img.shape[0]), (0, 0)), mode='constant', constant_values=1)
    ...
    def __call__(self, sample):
        ...
        try:
            np_style_img, np_gen_img = self.render_sample(sample, render_class)
        except OSError:
            try:
                print(f'WARNING rendering "{sample["style_text"]}" and/or "{sample["gen_text"]}" with font {self.ids_to_fonts[font_id].name}. Try to render only ascii letters.')
                self.clean_sample(sample)
                np_style_img, np_gen_img = self.render_sample(sample, render_class)
            except OSError:
                print(f'ERROR rendering "{sample["style_text"]}" and/or "{sample["gen_text"]}" with font {self.ids_to_fonts[font_id].name}. Rendering with empty text. Please consider removing this font.')
                print(f'\t- Font path: {self.ids_to_fonts[font_id].absolute()}')
                sample['style_text'] = ''
                sample['gen_text'] = ''
                np_style_img = np_gen_img = np.zeros((128, 64), dtype=np.uint8)
        except Exception as e:
            print(f'FATAL ERROR rendering "{sample["style_text"]}" and/or "{sample["gen_text"]}" with font {self.ids_to_fonts[font_id].name}.')
            print(f'\t- Font path: {self.ids_to_fonts[font_id].absolute()}')
            print(f'\t- Error: {e}')
            sample['style_text'] = ''
            sample['gen_text'] = ''
        ...
        np_style_img = self.resize_and_fill_with_white(np_style_img, max(np_style_img.shape[0], np_gen_img.shape[0]))
        np_gen_img = self.resize_and_fill_with_white(np_gen_img, max(np_style_img.shape[0], np_gen_img.shape[0]))
        np_img = np.concatenate([np_style_img, np.ones((np_style_img.shape[0], 20)), np_gen_img], axis=1)
        sample['style_img_width'] = np_style_img.shape[1]+10
        sample['gen_img_width'] = np_gen_img.shape[1]+10
