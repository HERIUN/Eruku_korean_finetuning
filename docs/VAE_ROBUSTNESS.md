...
- 진단도구: `_eval_vae_hw_recon.py`(실제 HW 복원 MSE/SSIM), `scratchpad/vae_sharpen.py`(농도 snap 테스트).
...
- 구현: `_eval_mirror_compare.py`(`bbox_crop_gray` 밀도기반, `--bbox-crop` 기본 on).
...
(`_eval_gen_heldout.py`, montage `finetune_runs/eruku_short_fullmse/_eval/gen_heldout_fonts.png`).
