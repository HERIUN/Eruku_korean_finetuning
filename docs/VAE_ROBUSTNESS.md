...
- **수정 방향(재학습 필요, 미적용)**: 타깃에 **입력과 매칭된 잉크 alpha** 부여(농담 보존 denoising —
  배경/블러만 제거하고 잉크농도는 유지). `custom_datasets.korean.aux._make_rgb`/`__getitem__` 에서 alpha 를
...
`custom_datasets.korean.split` 학습 파이프라인은 style 을 `TailorTensor(ink 타이트크롭) → ImgResize(64)` 로
