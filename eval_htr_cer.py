...
문제: eval_cer.py 의 easyocr 은 한글을 못 읽어 GT 렌더조차 CER 0.236 → 생성 개선을 측정 못 함
      (한글 gen CER 0.20 이 이미 OCR 바닥 아래라 saturated).
해결: train_aux_htr 로 만든 한글 HTR 은 렌더를 CER 0.00 로 읽음 → 바닥이 0 이라 실제 생성오차를 정밀 측정.

생성(reuse infer_show.gen_arr) → HTR greedy 읽기 → target 과 CER. GT 렌더도 HTR 로 읽어 '바닥' 동시 보고.
길이버킷·한/영 분해. (독립 교차검증은 easyocr 쓰는 eval_cer.py 병행.)

사용: CUDA_VISIBLE_DEVICES=2 python eval_htr_cer.py --ckpt <ckpt> [--vae-checkpoint <vae>] [--english] --n 100
"""
