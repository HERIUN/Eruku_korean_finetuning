...
기존 측정 산출물만 재분석한다(GPU 불필요, 새 생성 없음):
  --vae-json  : _eval_jong_stroke_loss.py 의 per_syllable 덤프  → H1
  --gen-json  : _eval_jong_gen_loss.py 의 per_syllable 덤프     → H2b
...
def train_exposure(seed=42):
    ...
    `korean_split_dataset.build_samplers` 의 구성을 그대로 재현해 해석적으로 계산한다:
...
def h1(vae_json, model="korean"):
    ...
    if not rows:
        print("[H1] per_syllable 덤프가 없다 — _eval_jong_stroke_loss.py 를 다시 돌릴 것"); return
