...
(생성물은 정렬이 안 돼 이렇게 못 잰다 → 그건 _eval_jong_gen_loss.py 에서 HTR 로 잰다).
...
사용:
  CUDA_VISIBLE_DEVICES=2 .venv/bin/python experiments/_eval_jong_stroke_loss.py \
...
def render64_with_spans(font_path, text, size=72, pad=14):
    """infer_show.render_in_font + eval 의 64px 정규화를 재현하되 글자별 x 범위도 반환.
