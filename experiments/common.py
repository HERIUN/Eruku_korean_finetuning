...
이 폴더의 스크립트는 `from common import …` 만으로 repo 루트가 sys.path 에 들어가
루트 모듈(infer.show, eval.htr_cer …)을 그대로 import 할 수 있다
...
def read_cer(htr, arr_u8, text, dev):
    ...
    from train.aux_htr import cer as char_cer
...
# ── montage 헬퍼 (생성 비교류: infer.show.cell 그리드) ─────────────────────────

def header_row(col_labels, cw, ch):
    """컬럼 라벨 헤더 한 줄."""
    from infer.show import cell
