...
def build_pools(corpus_json: Path, extra_chars: Path | None, english_path,
                n_english: int, rng: random.Random) -> dict:
    """ko(한글 단어)+en(영어 단어) 풀. 숫자/구두점/특수기호는 런타임 생성."""
    ko: set[str] = set()
    for r in json.loads(Path(corpus_json).read_text(encoding="utf-8")):
...
def main():
    ...
    ap.add_argument("--corpus", default=str(HERE / "assets" / "corpus" / "korean_lines.json"))
