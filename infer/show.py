...
Usage:
  python infer/show.py --ckpt finetune_runs/korean_v2/checkpoint_step_072000.pth \
...
def gen_arr(model, dec, ref_text, seed_text, cfg, max_new, style_px):
    ...
    (infer.show/infer.matrix 의 main). style 텐서에서 바로 가려면 gen_from_style.
    """
    with torch.no_grad():
...
def gen_from_style(model, style_img, style_text, gen_text, cfg, max_new, device,
    ...
    (infer.show/infer.matrix 의 main 참고 — 여긴 매 호출 VAE encode 를 다시 한다).
    """
