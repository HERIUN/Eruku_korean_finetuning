...
이 스크립트가 tar.gz 를 받아 `model_zoo/` 에 풀어, train/aux_htr.py / train/vae_korean.py 가
`HTR(**config)+load_state_dict(safetensors)` 로 warm-start 하도록 한다.

원본 HTR 은 라틴 전용(alphabet_size=167). 한글은 train/aux_htr.py 가 vocab 을 확장(warm-start)해 finetune.

사용:
  python tools/fetch_aux.py            # HTR 만
  python tools/fetch_aux.py --wid      # WriterID 도 (현재 파이프라인 미사용)
"""
