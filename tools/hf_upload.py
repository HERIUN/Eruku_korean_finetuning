...
사용법:
    ...
    .venv/bin/python tools/hf_upload.py model --verify-cer             # dry-run
...
# 상태로만 봤기 때문. 추론 단계 크롭으로 재학습 없이 해결된다(experiments/mirror_compare.py 검증).
BBOX_HELPER = '''
...
# 그 다음 열부터 자른다. 학습 harness 의 infer.show.gen_arr 와 같은 규약.
...
validation. See [`train/vae_korean.py`](https://github.com/HERIUN/Eruku_korean_finetuning).
...
def _verify_cer(hf_model, args):
    ...
    그래서 로컬 harness 수치(eval/htr_cer.py)와 같은 리더로 재서 임계 이내인지 확인한다.
...
def main():
    ...
    v.add_argument("--figures-dir", default=None,
                   help="카드에 넣을 figure 디렉토리. recon_{ko,en}.png(experiments/vae_recon.py --label-lang en) + "
                        "gencmp_{ko,en}_release.png(experiments/gen_compare.py --cats ko_release en_release). "
                        "있는 파일만 samples/ 로 올리고 카드에 삽입")
    # 모델카드 수치 (experiments/vae_recon.py 고정 probe 평균)
    ...
    m.add_argument("--figures-dir", default=None,
                   help="카드에 넣을 생성 비교 figure 디렉토리(gencmp_{ko,en}_release.png). "
                        "experiments/gen_compare.py --cats ko_release en_release 로 생성")
    ...
    # 모델카드 수치 (eval/htr_cer.py, n=100, cfg=1.0)
