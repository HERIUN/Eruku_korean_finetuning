...
최적화된 적이 없었다. → train_aux_htr.py 로 만든 **한글 HTR** 을 붙여 VAE 를 한글에 적응.
...
결과 `save_pretrained(out/vae_sXXXX)` → Eruku `--vae-checkpoint` 로 로드(교체 검증: infer_show/eval_cer 의 --vae-checkpoint).

예:
  python train_vae_korean.py --train-part decoder --htr-checkpoint finetune_runs/aux_htr_ko/htr_s20000 \
...
def make_parser():
    ...
    p.add_argument("--htr-checkpoint", required=True, help="train_aux_htr.py 결과(htr_sXXXX)")
